...
const SeatLayout = () => {
  ...
  const handleSeatClick = (seatId) => {
    ...
    setSelectedSeats((prev) => prev.includes(seatId) ? 
      // Nếu seatId đã được chọn, bỏ chọn nó
      prev.filter(seat => seat !== seatId) : 
      // Nếu seatId chưa được chọn, thêm nó vào danh sách đã chọn
      [...prev, seatId]
    );
  }
  ...
  const renderSeats = (row, count = 9) => (
    <div key={row} className="flex gap-2 mt-2">
      <div className="flex flex-wrap items-center justify-center gap-2">
        {/* Tạo một mảng undefined với số lượng count, quan tâm đến index */}
  ...
  // Suất chiếu khác nhau nên ghế bị chiếm dùng khác nhau
  useEffect(() => {
    if(selectedTime) {
      getOccupiedSeats()
    }
  ...
  return show ? (
    <div className='flex flex-col md:flex-row px-6 md:px-16 lg:px-40 pt-30 md:pt-50'> 
      ...
      {/* Seat Layout */}
      <div className='relative flex-1 flex flex-col items-center mã-md:mt:16'>
          ...
          <p className='tet-gray-400 text-sm mb-6'>
            SCREEN SIDE
          </p>
          ...
          <div className='flex flex-col items-center mt-10 text-sm text-gray-300'>
            ...
            {/* Xem lại nếu chưa chọn chỗ ngồi */}
